...
export type TokenChars = TypeTokenChars & ModifierTokenChars & DelimiterTokenChars;
export type CharOf<Token extends Enum> = TokenChars[Token];
type ReverseLookup<T, V> = {
    [P in keyof T]: T[P] extends V ? P : never;
}[keyof T];
export type TokenOf<Char extends string> = ReverseLookup<TokenChars, Char> extends never ? false : ReverseLookup<TokenChars, Char>;
