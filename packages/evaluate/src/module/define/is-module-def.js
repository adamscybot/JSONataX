...
export const InvalidJsonataModuleDefinition = customErrorFactory(function InvalidJsonataModuleDefinition(def, message = '') {
    this.code = 'INVALID_JSONATA_MODULE_DEFINITION';
    this.def = def;
    this.message =
        prefixedString(`Could not process module definition as it is invalid. Cause:`) +
            ' ' +
            message;
}, TypeError);
/**
 ...
 */
export function validateJsonataModuleDefFn(def) {
    const err = (message) => {
        throw InvalidJsonataModuleDefinition(def, 'Issue found in function definition: ' + message);
    };
    ...
    if (typeof def.name !== 'string' || !FN_NAME_REGEX.test(def.name))
        err("`name` must be a non-empty string consisting of alpha-numeric characters or '_', ':'.");
 ...
 */
export function isJsonataModuleDefFn(def) {
    try {
 ...
 * @throws {@link InvalidJsonataModuleDefinition}
 */
export function validateJsonataModuleDef(def) {
    const err = (message) => {
    ...
    if (typeof def.id !== 'string' || !MODULE_ID_REGEX.test(def.id))
        err("`id` must be a non-empty string consisting of alpha-numeric characters or '_'.");
    if (typeof def.description !== 'undefined' &&
        typeof def.description !== 'string')
        err('`description` must be a string or undefined.');
    if (!Array.isArray(def.exports))
        err('`fns` must be an array.');
}
/**
 ...
 * @returns `true` if valid, otherwise `false`
 */
export const isJsonataModuleDef = (def) => {
    try {
